...
class SimulationLoadFlow(SequentialTaskSet):
    ...
    def cycle_simulations(self) -> None:
        ...
        for _ in range(burst_size):
            ...
            self.user._get(
                "/simulation/history?limit=25", name="GET /simulation/history"
            )

    @task
    def inspect_simulation_state(self) -> None:
        self.user._get(
            "/simulation/state?include_topology=false", name="GET /simulation/state"
        )
...
class TrainingLoadFlow(SequentialTaskSet):
    ...
    def inspect_training_results(self) -> None:
        ...
        self.user._get(
            "/training/ppo/reward-curve", name="GET /training/ppo/reward-curve"
        )

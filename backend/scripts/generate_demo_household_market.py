...
def generate_household_csv(
    ...
    house_scale = np.clip(np_rng.normal(loc=1.0, scale=0.25, size=n_households), 0.55, 1.8)
    house_eff = np.clip(np_rng.normal(loc=1.0, scale=0.15, size=n_households), 0.6, 1.5)
    ...
    for i, t in enumerate(ts):
        ...
        per = np.clip(per * (1.0 + np_rng.normal(0.0, 0.08, size=n_households)), 0.0, None)
...
def generate_market_csv(
    ...
    supply = np.clip(0.78 * demand + np.random.default_rng(seed).normal(0, 3.5, size=len(demand)), 0.0, None)

    # price increases with demand and shortage
    shortage = np.clip(demand - supply, 0.0, None)
    price = 0.18 + 0.0042 * (demand / max(demand.max(), 1.0)) + 0.0065 * (shortage / max(shortage.max(), 1.0))
    price = np.clip(price + np.random.default_rng(seed + 1).normal(0, 0.01, size=len(price)), 0.05, 2.0)

    out = pd.DataFrame({"utc_timestamp": df_h["utc_timestamp"], "price": price.astype(np.float32)})
    if not price_only:
...
if __name__ == "__main__":
    ...
    household_out = backend_root / "data" / "historical_energy_data" / "demo_household_64_96steps.csv"
    market_price_only_out = backend_root / "data" / "market_prices" / "demo_market_price_only_96steps.csv"
    market_full_out = backend_root / "data" / "market_prices" / "demo_market_full_96steps.csv"
